...
const dz = async (path: string) => {
  try {
    const r = await fetch(`${CORS}${encodeURIComponent(DEEZER + path)}`);
    return r.json();
  } catch { return {}; }
};
const ytSearch = async (q: string) => {
...
const fmt = (s: number | undefined | null) => { if (!s && s !== 0) return "0:00"; return `${Math.floor(s / 60)}:${String(Math.floor(s % 60)).padStart(2, "0")}`; };
const fmtBig = (n: number) => n >= 1e6 ? `${(n / 1e6).toFixed(1)}M` : n >= 1e3 ? `${(n / 1e3).toFixed(0)}K` : String(n || 0);
...
const setToken = (t: string | null) => { try { if (t) localStorage.setItem("wave_sb_token", t); else localStorage.removeItem("wave_sb_token"); } catch {} };
...
const sbFetch = async (path: string, opts: RequestInit = {}) => {
  ...
  const r = await fetch(`${SB_URL}/rest/v1${path}`, {
    ...opts,
    headers: { "apikey": SB_KEY, "Authorization": `Bearer ${token}`, "Content-Type": "application/json", "Prefer": "return=representation", ...((opts.headers as Record<string,string>) || {}) },
  });
  if (!r.ok) return null;
...
/* ── YouTube URL Cache ── */
const ytCacheGet = async (trackId: number) => {
  const d = await sbFetch(`/yt_cache?track_id=eq.${trackId}&select=video_id&limit=1`);
  return d?.[0]?.video_id || null;
};
const ytCacheSet = async (trackId: number, artist: string, title: string, videoId: string) => {
...
const plCreate = async (name: string, isPublic = false) => {
  const uid = getUserId();
  return sbFetch("/user_playlists", { method: "POST", body: JSON.stringify({ name, is_public: isPublic, user_id: uid }) });
};
const plDelete = async (id: string) => { await sbFetch(`/user_playlists?id=eq.${id}`, { method: "DELETE" }); await sbFetch(`/playlist_tracks?playlist_id=eq.${id}`, { method: "DELETE" }); };
const plAddTrack = async (playlistId: string, track: Track) => sbFetch("/playlist_tracks", { method: "POST", body: JSON.stringify({ playlist_id: playlistId, track_id: track.id, track_data: track }) });
const plRemoveTrack = async (playlistId: string, trackId: number) => sbFetch(`/playlist_tracks?playlist_id=eq.${playlistId}&track_id=eq.${trackId}`, { method: "DELETE" });
const plGetTracks = async (playlistId: string) => { const d = await sbFetch(`/playlist_tracks?playlist_id=eq.${playlistId}&select=track_data&order=created_at.asc`); return (d || []).map((r: { track_data: Track }) => r.track_data); };

/* ── Similar Tracks Cache ── */
const similarGet = async (trackId: number) => { const d = await sbFetch(`/similar_cache?track_id=eq.${trackId}&select=tracks&limit=1`); return d?.[0]?.tracks || null; };
const similarSet = async (trackId: number, tracks: Track[]) => sbFetch("/similar_cache", { method: "POST", body: JSON.stringify({ track_id: trackId, tracks }) });

/* ── User Data Sync (liked, recent, prefs) ── */
const userDataGet = async (): Promise<{ liked: Track[]; recent: Track[]; prefs: Record<string,unknown> } | null> => {
  const uid = getUserId();
  if (!uid) return null;
  const d = await sbFetch(`/user_data?user_id=eq.${uid}&select=liked,recent,prefs&limit=1`);
  return d?.[0] || null;
};
const userDataUpsert = async (liked: Track[], recent: Track[], prefs: Record<string,unknown>) => {
  const uid = getUserId();
  if (!uid) return;
  await sbFetch("/user_data", {
    method: "POST",
    headers: { "Prefer": "resolution=merge-duplicates,return=minimal" },
    body: JSON.stringify({ user_id: uid, liked, recent, prefs, updated_at: new Date().toISOString() }),
  });
};

/* ─── LOCALSTORAGE PERSISTENCE ────────────────────────────────── */
const LS_PREFIX = "wave_";
const lsGet = <T,>(key: string, fallback: T): T => { try { const v = localStorage.getItem(LS_PREFIX + key); return v !== null ? JSON.parse(v) : fallback; } catch { return fallback; } };
const lsSet = (key: string, val: unknown) => { try { localStorage.setItem(LS_PREFIX + key, JSON.stringify(val)); } catch {} };
...
function ensureYT() {
  ...
  (window as Window & { onYouTubeIframeAPIReady?: () => void }).onYouTubeIframeAPIReady = () => { _ytReady = true; _ytCbs.forEach(c => c()); _ytCbs.length = 0; };
  const s = document.createElement("script"); s.src = "https://www.youtube.com/iframe_api"; document.head.appendChild(s);
}
function whenYT(cb: () => void) { if (_ytReady && (window as Window & { YT?: { Player: unknown } }).YT?.Player) cb(); else _ytCbs.push(cb); }
...
interface Track {
  ...
  rank?: number;
  release_date?: string;
  explicit_lyrics?: boolean | number;
  explicit_content_lyrics?: number;
  artist?: { id: number; name: string; picture?: string; picture_medium?: string; picture_big?: string; nb_fan?: number };
  album?: { id: number; title: string; cover?: string; cover_small?: string; cover_medium?: string; cover_big?: string; cover_xl?: string; release_date?: string };
}
interface Playlist {
  id: string;
  name: string;
...
.topbar { position: sticky; top: 0; z-index: 30; height: 48px; background: rgba(255,255,255,.88); backdrop-filter: blur(12px) saturate(180%); border-bottom: var(--line); display: flex; align-items: center; padding: 0 16px; gap: 10px; }
...
/* FULLSCREEN */
.fs-overlay { position: fixed; inset: 0; z-index: 200; background: #09090b; display: grid; grid-template-rows: 1fr auto; overflow: hidden; animation: fsIn .3s cubic-bezier(.4,0,.2,1); }
@keyframes fsIn { from { opacity: 0; transform: scale(1.015); } to { opacity: 1; transform: none; } }
.fs-bg { position: absolute; inset: -40px; background-size: cover; background-position: center; filter: blur(60px) saturate(1.4); opacity: .18; pointer-events: none; transition: background-image .6s; }
.fs-main { display: grid; grid-template-columns: 1fr 0px; gap: 0; overflow: hidden; position: relative; transition: grid-template-columns .25s cubic-bezier(.4,0,.2,1); }
.fs-main.sidebar-open { grid-template-columns: 1fr 300px; }
.fs-left { display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 40px 48px; border-right: 1px solid rgba(255,255,255,.07); position: relative; gap: 28px; }
.fs-art-wrap { position: relative; flex-shrink: 0; display: flex; align-items: center; gap: 20px; width: 100%; justify-content: center; }
.fs-art { width: clamp(200px, 26vw, 320px); height: clamp(200px, 26vw, 320px); border-radius: 6px; object-fit: cover; border: 1px solid rgba(255,255,255,.12); display: block; transition: transform .4s cubic-bezier(.4,0,.2,1), box-shadow .4s; }
.fs-art.playing { box-shadow: 0 0 0 1px rgba(255,255,255,.15), 0 24px 64px rgba(0,0,0,.6); transform: scale(1.02); }
.fs-art-placeholder { width: clamp(200px, 26vw, 320px); height: clamp(200px, 26vw, 320px); border-radius: 6px; border: 1px solid rgba(255,255,255,.1); display: flex; align-items: center; justify-content: center; background: #18181b; }
.fs-vinyl-waves { flex: 1; height: clamp(140px, 18vw, 240px); pointer-events: none; display: flex; align-items: center; justify-content: space-evenly; flex-shrink: 1; min-width: 0; overflow: hidden; }
.fs-vinyl-wave-bar { flex: 1; max-width: 14px; min-width: 8px; margin: 0 4px; height: 60%; border-radius: 4px; background: rgba(255,255,255,.22); transform-origin: center; animation: waveBar 1.1s ease-in-out infinite alternate; animation-play-state: paused; }
...
.fs-track-title { font-size: clamp(16px, 2vw, 24px); font-weight: 700; letter-spacing: -.03em; color: #fafafa; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; margin-bottom: 5px; }
...
.fs-action-btn { background: none; border: 1px solid rgba(255,255,255,.12); border-radius: 4px; width: 30px; height: 30px; display: flex; align-items: center; justify-content: center; cursor: pointer; color: rgba(255,255,255,.5); transition: all .1s; }
...
.fs-tab { font-family: 'Geist Mono', monospace; font-size: 10px; font-weight: 600; letter-spacing: .1em; text-transform: uppercase; color: rgba(255,255,255,.3); padding: 14px 12px; cursor: pointer; border-bottom: 2px solid transparent; margin-bottom: -1px; transition: all .1s; }
...
.fs-controls { border-top: 1px solid rgba(255,255,255,.08); padding: 14px 32px 18px; background: rgba(9,9,11,.7); backdrop-filter: blur(20px); flex-shrink: 0; }
...
.fs-sidebar-toggle { position: absolute; top: 14px; left: 14px; z-index: 10; background: rgba(255,255,255,.06); border: 1px solid rgba(255,255,255,.12); border-radius: 4px; width: 30px; height: 30px; display: flex; align-items: center; justify-content: center; cursor: pointer; color: rgba(255,255,255,.5); transition: all .1s; }
...
.auth-modal { background: var(--bg); border: 2px solid var(--tx); border-radius: var(--r); width: min(400px, 94vw); box-shadow: var(--shadow-lg); overflow: hidden; position: relative; }
.auth-modal-close { position: absolute; top: 12px; right: 12px; background: none; border: var(--line); border-radius: var(--r2); width: 26px; height: 26px; display: flex; align-items: center; justify-content: center; cursor: pointer; color: var(--tx3); z-index: 10; transition: all .1s; }
...
.user-avatar { width: 20px; height: 20px; border-radius: 50%; background: var(--tx); color: var(--bg); display: flex; align-items: center; justify-content: center; font-size: 9px; font-weight: 700; flex-shrink: 0; overflow: hidden; }
.user-avatar img { width: 100%; height: 100%; object-fit: cover; }

/* DOWNLOAD */
.dl-btn { display: inline-flex; align-items: center; gap: 5px; background: none; border: var(--line); border-radius: var(--r2); padding: 4px 10px; font-size: 11px; font-family: 'Geist Mono', monospace; color: var(--tx3); cursor: pointer; transition: all .1s; white-space: nowrap; }
.dl-btn:hover { color: var(--tx); background: var(--bg3); border-color: var(--border2); }
.dl-notice { font-size: 10px; color: var(--tx3); font-family: 'Geist Mono', monospace; opacity: .7; }
...
.mobile-nav { display: none; grid-column: 1/3; background: var(--surface); border-top: var(--line); align-items: stretch; }
.mobile-nav-item { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 3px; cursor: pointer; color: var(--tx3); transition: color .1s; padding: 6px 0; font-family: 'Geist Mono', monospace; font-size: 9px; font-weight: 600; letter-spacing: .05em; text-transform: uppercase; border: none; background: none; }
...
function useHashView(): [string, (v: string) => void] {
  const [view, setViewState] = useState(getHashView);
  const setView = useCallback((v: string) => { window.location.hash = v; }, []);
  useEffect(() => {
...
/* ─── AUTH MODAL ────────────────────────────────────────────────── */
const AuthModal = memo(({ onClose }: { onClose: () => void }) => {
      ...
      <div className="auth-modal" style={{ textAlign: "center" }} onClick={e => e.stopPropagation()}>
        ...
        <div style={{ padding: "24px 28px 28px", display: "flex", flexDirection: "column", gap: 10 }}>
          <button
            ...
            disabled={loading}
            style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, width: "100%", padding: "11px 16px", background: "var(--bg3)", border: "var(--line)", borderRadius: "var(--r2)", cursor: "pointer", fontSize: 13, fontWeight: 600, fontFamily: "'Geist',sans-serif", color: "var(--tx)", transition: "all .1s" }}
          >
...
/* ─── COMMAND PALETTE ───────────────────────────────────────────── */
const CommandPalette = memo(({ onClose, recent, liked, charts, onPlay, onNav, onAction }: {
  onClose: () => void; recent: Track[]; liked: Track[]; charts: Track[];
  onPlay: (t: Track, list: Track[], i: number) => void; onNav: (v: string) => void; onAction: (a: string) => void;
}) => {
  ...
  const flat: (typeof actions[0] | Track)[] = [...filtered.actions, ...filtered.tracks];
  ...
  useEffect(() => {
    const fn = (e: KeyboardEvent) => {
      ...
      if (e.key === "Enter") {
        ...
        const item = flat[sel];
        if (!item) return;
        if ("action" in item) { item.action(); onClose(); }
        else { onPlay(item as Track, [item as Track], 0); onClose(); }
      }
    };
    window.addEventListener("keydown", fn);
    return () => window.removeEventListener("keydown", fn);
      ...
      <div className="cmd-box">
        ...
        <div className="cmd-results">
          {filtered.actions.length > 0 && <>
            ...
            {filtered.actions.map(a => { const i = idx++; return <div key={a.label} className={`cmd-item${sel === i ? " selected" : ""}`} onMouseEnter={() => setSel(i)} onClick={() => { a.action(); onClose(); }}><div className="cmd-item-icon"><span style={{ fontSize: 12 }}>{a.icon}</span></div><div className="cmd-item-meta"><div className="cmd-item-label">{a.label}</div>{a.sub && <div className="cmd-item-sub">{a.sub}</div>}</div></div>; })}
          </>}
          {filtered.tracks.length > 0 && <>
            <div className="cmd-section">{q ? "Tracks" : "Recently played"}</div>
            {filtered.tracks.map(t => { const i = idx++; return <div key={t.id} className={`cmd-item${sel === i ? " selected" : ""}`} onMouseEnter={() => setSel(i)} onClick={() => { onPlay(t, [t], 0); onClose(); }}>{t.album?.cover_small ? <img className="cmd-item-img" src={t.album.cover_small} alt="" /> : <div className="cmd-item-icon"><svg width="12" height="12" fill="currentColor" viewBox="0 0 24 24"><path d="M9 18V5l12-2v13" /></svg></div>}<div className="cmd-item-meta"><div className="cmd-item-label">{t.title}</div><div className="cmd-item-sub">{t.artist?.name}</div></div><span className="cmd-item-kbd">{fmt(t.duration)}</span></div>; })}
          </>}
...
/* ─── LYRICS PAGE ───────────────────────────────────────────────── */
const LyricsPage = memo(({ current, ytRef }: { current: Track | null; ytRef: React.RefObject<{ getCurrentTime?: () => number } | null> }) => {
    ...
    fetch(`https://lrclib.net/api/get?artist_name=${encodeURIComponent(current.artist?.name || "")}&track_name=${encodeURIComponent(current.title || "")}&duration=${current.duration || 0}`)
      ...
      .then(d => {
        if (d.syncedLyrics) {
          ...
          setSynced(parsed);
        } else if (d.plainLyrics) { setPlain(d.plainLyrics.split("\n")); }
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      let idx = 0;
      for (let i = 0; i < synced.length; i++) { if (synced[i].time <= t) idx = i; else break; }
  ...
  return (
    <div className="lyrics-page">
      ...
      {current && <div className="lyrics-header">
        {current.album?.cover_small && <img className="lyrics-header-art" src={current.album.cover_small} alt="" />}
        <div className="lyrics-header-meta"><div className="lyrics-header-title">{current.title}</div><div className="lyrics-header-artist">{current.artist?.name}</div></div>
        {synced && <span style={{ fontFamily: "'Geist Mono',monospace", fontSize: 10, color: "rgba(255,255,255,.3)", flexShrink: 0 }}>● synced</span>}
      </div>}
      {!current && <div className="lyrics-idle"><svg width="24" height="24" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path d="M9 18V5l12-2v13" /><circle cx="6" cy="18" r="3" /><circle cx="18" cy="16" r="3" /></svg><span>play a track to see lyrics</span></div>}
      {current && loading && <div className="lyrics-loading">fetching lyrics…</div>}
      {current && !loading && !synced && !plain && <div className="lyrics-idle"><span>no lyrics found for this track</span></div>}
      {current && synced && <div className="lyrics-body"><div className="lyrics-scroll" ref={scrollRef}>{synced.map((l, i) => <div key={i} ref={el => { lineRefs.current[i] = el; }} className={`lyrics-line${i === activeIdx ? " active" : Math.abs(i - activeIdx) === 1 ? " near" : ""}`}>{l.text || <span>&nbsp;</span>}</div>)}</div></div>}
      {current && !synced && plain && <div className="lyrics-body"><div className="lyrics-scroll" ref={scrollRef}>{plain.map((l, i) => <div key={i} className={`lyrics-line${!l.trim() ? " empty" : ""}`}>{l || <span>&nbsp;</span>}</div>)}</div></div>}
    </div>
  );
});

/* ─── STATS PAGE ────────────────────────────────────────────────── */
const StatsPage = memo(({ recent, liked }: { recent: Track[]; liked: Track[] }) => {
  ...
  return (
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", borderLeft: "var(--line)" }}>
        {[{ val: recent.length, key: "Tracks played" }, { val: totalMin, key: "Minutes listened" }, { val: liked.length, key: "Saved tracks" }, { val: new Set(recent.map(t => t.artist?.name)).size, key: "Unique artists" }, { val: new Set(recent.map(t => t.album?.title)).size, key: "Unique albums" }, { val: Math.round(totalMin / 60 * 10) / 10, key: "Hours total" }].map(s => <div key={s.key} className="stats-cell" style={{ borderRight: "var(--line)", borderBottom: "var(--line)" }}><div className="stats-cell-val">{s.val}</div><div className="stats-cell-key">{s.key}</div></div>)}
      </div>
      {topArtists.length > 0 && <><div className="sec-hd"><span className="sec-lbl">Top Artists</span></div>{topArtists.map(([name, count]) => <div key={name} className="stats-bar-row"><div className="stats-bar-label">{name}</div><div className="stats-bar-track"><div className="stats-bar-fill" style={{ width: `${(count / maxCount) * 100}%` }} /></div><div className="stats-bar-count">{count}</div></div>)}</>}
...
const WaveLogo = ({ size = 20 }: { size?: number }) => {
  ...
  return <div className="logo-mark" style={{ height: size }}>{bars.map((h, i) => <span key={i} style={{ width: 2.5, height: h * (size / 20), borderRadius: 1.5 }} />)}</div>;
};
...
const CardGrid = ({ items, onClick, imgFn, titleFn, subFn, round = false }: {
  items: (Track | Playlist)[]; onClick: (item: Track | Playlist) => void; imgFn: (item: Track | Playlist) => string; titleFn: (item: Track | Playlist) => string; subFn: (item: Track | Playlist) => string; round?: boolean;
}) => (
  <div className="cgrid">{items.map(item => <div key={(item as Track | Playlist).id} className="gc" onClick={() => onClick(item)}>
    <img className={`gc-img${round ? " round" : ""}`} src={imgFn(item) || ""} alt="" />
    <div className="gc-title">{titleFn(item)}</div><div className="gc-sub">{subFn(item)}</div>
    <div className="gc-play"><svg width="11" height="11" fill="var(--bg)" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg></div>
  </div>)}</div>
);

/* ─── TRACK ROW ─────────────────────────────────────────────────── */
const TRow = memo(({ track, index, list, showAlbum, currentId, playing, buffering, liked, user, onPlay, onLike, onAddToPlaylist, onSimilar, onDownload, onAuthRequired }: {
  track: Track; index: number; list: Track[]; showAlbum?: boolean; currentId?: number; playing: boolean; buffering: boolean;
  liked: Track[]; user: UserInfo | null; onPlay: (t: Track, l: Track[], i: number) => void; onLike: (t: Track) => void;
  onAddToPlaylist?: (t: Track, e: React.MouseEvent) => void; onSimilar?: (t: Track) => void; onDownload?: (t: Track) => void; onAuthRequired: () => void;
}) => {
  ...
  return (
    <tr className={active ? "tr-active" : ""} onClick={() => onPlay(track, list || [track], index)}>
      <td className="td-n">
        {active && playing ? <div className="eq" style={{ margin: "0 auto" }}><div className="eq-b" /><div className="eq-b" /><div className="eq-b" /></div>
          : active && buffering ? <svg className="spin" width="12" height="12" fill="none" stroke="var(--accent)" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M21 12a9 9 0 11-6.219-8.56" /></svg>
          : <span style={{ fontFamily: "'Geist Mono',monospace", fontSize: 11 }}>{index + 1}</span>}
      </td>
      <td><div className="td-info">
        {track.album?.cover_small ? <img className="td-art" src={track.album.cover_small} alt="" /> : null}
        <div style={{ overflow: "hidden" }}>
          <div className="td-name"><span style={{ overflow: "hidden", textOverflow: "ellipsis" }}>{track.title}</span>{isExplicit ? <span className="explicit-tag">E</span> : null}</div>
      ...
      <td style={{ textAlign: "right", whiteSpace: "nowrap" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 2 }}>
          {onSimilar && <button className="like-btn" title="Similar tracks" onClick={e => { e.stopPropagation(); onSimilar(track); }}><svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="3" /><path d="M12 3a9 9 0 0 1 9 9M3 12a9 9 0 0 1 9-9" /></svg></button>}
          {onAddToPlaylist && <button className="like-btn" title="Add to playlist" onClick={e => { e.stopPropagation(); if (!user) { onAuthRequired(); return; } onAddToPlaylist(track, e); }}><svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M12 5v14M5 12h14" /></svg></button>}
          {onDownload && track.preview && <button className="like-btn" title="Download 30s preview" onClick={e => { e.stopPropagation(); if (!user) { onAuthRequired(); return; } onDownload(track); }}><svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" /><polyline points="7 10 12 15 17 10" /><line x1="12" y1="15" x2="12" y2="3" /></svg></button>}
          <button className={`like-btn${isLiked ? " liked" : ""}`} onClick={handleLike}>
            <svg width="13" height="13" fill={isLiked ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" /></svg>
          </button>
        </div>
      </td>
    </tr>
...
/* ─── TRACK TABLE ───────────────────────────────────────────────── */
const TTable = memo(({ tracks, showAlbum = true, explicitFilter, currentId, playing, buffering, liked, user, onPlay, onLike, onAddToPlaylist, onSimilar, onDownload, onAuthRequired }: {
  tracks: Track[]; showAlbum?: boolean; explicitFilter: boolean; currentId?: number; playing: boolean; buffering: boolean;
  liked: Track[]; user: UserInfo | null; onPlay: (t: Track, l: Track[], i: number) => void; onLike: (t: Track) => void;
  onAddToPlaylist?: (t: Track, e: React.MouseEvent) => void; onSimilar?: (t: Track) => void; onDownload?: (t: Track) => void; onAuthRequired: () => void;
}) => {
  const filtered = explicitFilter ? tracks.filter(t => t.explicit_lyrics !== 1 && t.explicit_lyrics !== true && t.explicit_content_lyrics !== 1) : tracks;
  if (!filtered.length) return <div className="empty"><div className="empty-title">No tracks</div><div className="empty-sub">{explicitFilter ? "All results filtered (explicit filter on)" : "Nothing here yet"}</div></div>;
  return (
    <table className="ttbl">
      <thead><tr><th style={{ width: 36 }}>#</th><th>TITLE</th>{showAlbum ? <th>ALBUM</th> : null}<th className="r">TIME</th><th style={{ width: 36 }}></th></tr></thead>
      <tbody>{filtered.map((t, i) => <TRow key={t.id} track={t} index={i} list={filtered} showAlbum={showAlbum} currentId={currentId} playing={playing} buffering={buffering} liked={liked} user={user} onPlay={onPlay} onLike={onLike} onAddToPlaylist={onAddToPlaylist} onSimilar={onSimilar} onDownload={onDownload} onAuthRequired={onAuthRequired} />)}</tbody>
    </table>
  );
});

/* ─── WAVE BARS ─────────────────────────────────────────────────── */
const WaveBars = memo(({ playing }: { playing: boolean }) => (
  <div className={`fs-vinyl-waves${playing ? " playing" : ""}`}>{Array.from({ length: 8 }, (_, i) => <div key={i} className="fs-vinyl-wave-bar" />)}</div>
));

/* ─── FS PROGRESS BAR ──────────────────────────────────────────── */
const FsProgressBar = memo(({ playing, currentId, currentDuration, ytRef, onSeek }: {
  playing: boolean; currentId?: number; currentDuration?: number;
  ytRef: React.RefObject<{ getCurrentTime?: () => number; getDuration?: () => number } | null>; onSeek: (t: number) => void;
}) => {
  const [progress, setProgress] = useState(0);
  const [duration, setDuration] = useState(currentDuration || 0);
  const timerRef = useRef<ReturnType<typeof setInterval> | null>(null);
  const fmtL = (s: number) => { if (!s && s !== 0) return "0:00"; return `${Math.floor(s / 60)}:${String(Math.floor(s % 60)).padStart(2, "0")}`; };
  ...
  const handleSeek = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const t = p * (duration || 0); setProgress(t); onSeek(t);
  };
  return (
    <div className="fs-prog-row">
      <span className="fs-prog-time">{fmtL(progress)}</span>
      <div className="fs-prog-rail" onClick={handleSeek}><div className="fs-prog-fill" style={{ width: `${pct}%` }} /><div className="fs-prog-thumb" style={{ left: `${pct}%` }} /></div>
      <span className="fs-prog-time" style={{ textAlign: "right" }}>{fmtL(duration)}</span>
    </div>
  );
});

/* ─── FULLSCREEN VIEW ──────────────────────────────────────────── */
const FullscreenView = memo(({ current, queue, qIdx, playing, buffering, shuffle, repeat, liked, volume, ytRef, user, onClose, onTogglePlay, onAdvance, onSeek, onVolume, onLike, onShuffle, onRepeat, onPlayFromQueue, relatedTracks, onAuthRequired }: {
  current: Track | null; queue: Track[]; qIdx: number; playing: boolean; buffering: boolean; shuffle: boolean; repeat: boolean;
  liked: Track[]; volume: number; ytRef: React.RefObject<{ getCurrentTime?: () => number; getDuration?: () => number } | null>;
  user: UserInfo | null; onClose: () => void; onTogglePlay: () => void; onAdvance: (d: number) => void;
  onSeek: (t: number) => void; onVolume: (v: number) => void; onLike: (t: Track) => void;
  onShuffle: () => void; onRepeat: () => void; onPlayFromQueue: (t: Track, i: number) => void;
  relatedTracks: Track[]; onAuthRequired: () => void;
}) => {
  ...
  useEffect(() => {
    const fn = (e: KeyboardEvent) => { if (e.key === "Escape" || e.code === "KeyF") onClose(); };
    window.addEventListener("keydown", fn);
    return () => window.removeEventListener("keydown", fn);
  }, [onClose]);
  ...
  const coverUrl = current?.album?.cover_xl || current?.album?.cover_big || current?.album?.cover_medium || "";
  return (
    <div className="fs-overlay" role="dialog">
      {coverUrl && <div className="fs-bg" style={{ backgroundImage: `url(${coverUrl})` }} />}
      <button className="fs-close" onClick={onClose}><svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M18 6 6 18M6 6l12 12" /></svg></button>
      <div className={`fs-main${sidebarOpen ? " sidebar-open" : ""}`}>
        <div className="fs-left">
          <button className={`fs-sidebar-toggle${sidebarOpen ? " open" : ""}`} onClick={() => setSidebarOpen(s => !s)}>
            <svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M8 6h13M8 12h13M8 18h13M3 6h.01M3 12h.01M3 18h.01" /></svg>
          </button>
          <div className="fs-art-wrap">
            <WaveBars playing={playing} />
            {coverUrl ? <img className={`fs-art${playing ? " playing" : ""}`} src={coverUrl} alt="" /> : <div className="fs-art-placeholder"><svg width="48" height="48" fill="none" stroke="rgba(255,255,255,.2)" strokeWidth="1.5" viewBox="0 0 24 24"><path d="M9 18V5l12-2v13" /><circle cx="6" cy="18" r="3" /><circle cx="18" cy="16" r="3" /></svg></div>}
            <WaveBars playing={playing} />
          </div>
          <div className="fs-meta">
            <div className="fs-track-title">{current?.title || "No track playing"}</div>
            <div className="fs-track-artist">{current?.artist?.name || "—"}</div>
            {current?.album?.title && <div className="fs-track-album">{current.album.title}{current.album.release_date ? ` · ${current.album.release_date.slice(0, 4)}` : ""}</div>}
            <div className="fs-actions">
              <button className={`fs-action-btn${isLiked ? " liked" : ""}`} onClick={() => { if (!user) { onAuthRequired(); return; } if (current) onLike(current); }}>
                <svg width="13" height="13" fill={isLiked ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" /></svg>
              </button>
              {current?.link && <button className="fs-action-btn" onClick={() => window.open(current.link, "_blank")}><svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6" /><polyline points="15 3 21 3 21 9" /><line x1="10" y1="14" x2="21" y2="3" /></svg></button>}
              <button className={`fs-action-btn${shuffle ? " active" : ""}`} onClick={onShuffle}><svg width="13" height="13" fill="currentColor" viewBox="0 0 24 24"><path d="M16 3h5v5l-1.5-1.5-4.538 4.538-1.414-1.414L17.5 5.5 16 4V3zM4 5l6.5 6.5-1.414 1.415L3 6.415 4 5zm10.5 9.5L9 10 7.5 11.5 13 17l1.5-1.5zm-4.538-4.538L3 17h2v1l6.5-6.5-1.538-1.538zM16 16l1.5-1.5 3.5 3.5V19h-5v-1l1.5-1.5L16 16z" /></svg></button>
              <button className={`fs-action-btn${repeat ? " active" : ""}`} onClick={onRepeat}><svg width="13" height="13" fill="currentColor" viewBox="0 0 24 24"><path d="M7 7h10v3l4-4-4-4v3H5v6h2V7zm10 10H7v-3l-4 4 4 4v-3h12v-6h-2v4z" /></svg></button>
            </div>
          </div>
        </div>
        <div className="fs-right">
          <div className="fs-tabs">{[{ id: "queue", label: `Queue (${queue.length})` }, { id: "details", label: "Details" }, { id: "related", label: "Related" }].map(t => <div key={t.id} className={`fs-tab${activeTab === t.id ? " active" : ""}`} onClick={() => setActiveTab(t.id)}>{t.label}</div>)}</div>
          {activeTab === "queue" && <div className="fs-queue">{queue.length === 0 ? <div style={{ padding: "40px 20px", textAlign: "center", color: "rgba(255,255,255,.25)", fontSize: 12, fontFamily: "'Geist Mono',monospace" }}>no tracks in queue</div> : queue.map((t, i) => <div key={`${t.id}-${i}`} className={`fs-queue-item${i === qIdx ? " current" : ""}`} onClick={() => onPlayFromQueue(t, i)}><span className="fs-queue-num">{i === qIdx && playing ? <div className="fs-eq"><div className="fs-eq-b" /><div className="fs-eq-b" /><div className="fs-eq-b" /></div> : i + 1}</span><img src={t.album?.cover_small || ""} alt="" /><div className="fs-queue-meta"><div className={`fs-queue-name${i === qIdx ? " active" : ""}`}>{t.title}</div><div className="fs-queue-sub">{t.artist?.name}</div></div><span className="fs-queue-dur">{fmt(t.duration)}</span></div>)}</div>}
          {activeTab === "details" && <div className="fs-details">{!current ? <div style={{ color: "rgba(255,255,255,.25)", fontSize: 12, fontFamily: "'Geist Mono',monospace" }}>no track selected</div> : <>{[{ k: "Title", v: current.title }, { k: "Artist", v: current.artist?.name }, { k: "Album", v: current.album?.title }, { k: "Year", v: current.album?.release_date?.slice(0, 4) || current.release_date?.slice(0, 4) }, { k: "Duration", v: fmt(current.duration) }, { k: "BPM", v: current.bpm ? `${current.bpm} bpm` : null }, { k: "Rank", v: current.rank ? `#${current.rank.toLocaleString()}` : null }, { k: "Track ID", v: String(current.id) }].filter(r => r.v).map(row => <div key={row.k} className="fs-detail-row"><div className="fs-detail-key">{row.k}</div><div className="fs-detail-val">{row.v}</div></div>)}</>}</div>}
          {activeTab === "related" && <div className="fs-related">{relatedTracks.length === 0 ? <div style={{ padding: "40px 20px", textAlign: "center", color: "rgba(255,255,255,.25)", fontSize: 12, fontFamily: "'Geist Mono',monospace" }}>play a track to see related</div> : relatedTracks.map((t, i) => <div key={t.id} className="fs-related-item" onClick={() => onPlayFromQueue(t, i)}><img src={t.album?.cover_small || ""} alt="" /><div className="fs-related-meta"><div className="fs-related-name">{t.title}</div><div className="fs-related-sub">{t.artist?.name} · {fmt(t.duration)}</div></div></div>)}</div>}
        </div>
      </div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", position: "relative" }}>
          <div className="fs-side-controls">
            <div className="fs-vol-wrap">
              <svg width="12" height="12" fill="currentColor" style={{ color: "rgba(255,255,255,.3)", flexShrink: 0 }} viewBox="0 0 24 24">{volume === 0 ? <path d="M16.5 12c0-1.77-1.02-3.29-2.5-4.03v2.21l2.45 2.45c.03-.2.05-.41.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51C20.63 14.91 21 13.5 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06c1.38-.31 2.63-.95 3.69-1.81L19.73 21 21 19.73l-9-9L4.27 3zM12 4L9.91 6.09 12 8.18V4z" /> : volume < 50 ? <path d="M18.5 12c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM5 9v6h4l5 5V4L9 9H5z" /> : <path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z" />}</svg>
          ...
          <div className="fs-btn-row" style={{ position: "absolute", left: "50%", transform: "translateX(-50%)" }}>
            <button className={`fs-ctrl${shuffle ? " active" : ""}`} onClick={onShuffle}><svg width="16" height="16" fill="currentColor" viewBox="0 0 24 24"><path d="M16 3h5v5l-1.5-1.5-4.538 4.538-1.414-1.414L17.5 5.5 16 4V3zM4 5l6.5 6.5-1.414 1.415L3 6.415 4 5zm10.5 9.5L9 10 7.5 11.5 13 17l1.5-1.5zm-4.538-4.538L3 17h2v1l6.5-6.5-1.538-1.538zM16 16l1.5-1.5 3.5 3.5V19h-5v-1l1.5-1.5L16 16z" /></svg></button>
            <button className="fs-ctrl" onClick={() => onAdvance(-1)}><svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24"><path d="M6 6h2v12H6zm3.5 6 8.5 6V6z" /></svg></button>
            <button className="fs-play-btn" onClick={onTogglePlay}>{buffering ? <svg className="spin" width="14" height="14" fill="none" stroke="#09090b" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M21 12a9 9 0 11-6.219-8.56" /></svg> : playing ? <svg width="14" height="14" fill="#09090b" viewBox="0 0 24 24"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z" /></svg> : <svg width="14" height="14" fill="#09090b" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>}</button>
            <button className="fs-ctrl" onClick={() => onAdvance(1)}><svg width="20" height="20" fill="currentColor" viewBox="0 0 24 24"><path d="M6 18l8.5-6L6 6v12zm10-12v12h2V6h-2z" /></svg></button>
            <button className={`fs-ctrl${repeat ? " active" : ""}`} onClick={onRepeat}><svg width="16" height="16" fill="currentColor" viewBox="0 0 24 24"><path d="M7 7h10v3l4-4-4-4v3H5v6h2V7zm10 10H7v-3l-4 4 4 4v-3h12v-6h-2v4z" /></svg></button>
          </div>
          <div className="fs-side-controls" style={{ justifyContent: "flex-end" }}>
            ...
            <button className="fs-action-btn" onClick={onClose} style={{ marginLeft: 8 }}><svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M8 3v3a2 2 0 0 1-2 2H3m18 0h-3a2 2 0 0 1-2-2V3m0 18v-3a2 2 0 0 1 2-2h3M3 16h3a2 2 0 0 1 2 2v3" /></svg></button>
          </div>
        </div>
      </div>
    </div>
...
/* ─── PLAYER BAR ────────────────────────────────────────────────── */
const PlayerBar = memo(({ current, playing, buffering, shuffle, repeat, liked, volume, user, onTogglePlay, onAdvance, onSeek, onVolume, onLike, onShuffle, onRepeat, onFullscreen, ytRef, onAuthRequired }: {
  current: Track | null; playing: boolean; buffering: boolean; shuffle: boolean; repeat: boolean; liked: Track[]; volume: number; user: UserInfo | null;
  onTogglePlay: () => void; onAdvance: (d: number) => void; onSeek: (t: number) => void; onVolume: (v: number) => void;
  onLike: (t: Track) => void; onShuffle: () => void; onRepeat: () => void; onFullscreen: () => void;
  ytRef: React.RefObject<{ getCurrentTime?: () => number; getDuration?: () => number; getPlayerState?: () => number } | null>; onAuthRequired: () => void;
}) => {
  const [progress, setProgress] = useState(0);
    ...
    <div className="player">
      <div className="pl-track">
        {current ? <>
          ...
          <div className="pl-meta"><div className="pl-title">{current.title}</div><div className="pl-artist">{current.artist?.name}</div></div>
          <button className={`like-btn${isLiked ? " liked" : ""}`} onClick={() => { if (!user) { onAuthRequired(); return; } onLike(current); }} style={{ flexShrink: 0, marginLeft: 6 }}>
            <svg width="13" height="13" fill={isLiked ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" /></svg>
          </button>
      ...
      <div className="pl-center">
        <div className="pl-btns">
          <button className={`ctrl-btn${shuffle ? " active" : ""}`} onClick={onShuffle}><svg width="14" height="14" fill="currentColor" viewBox="0 0 24 24"><path d="M16 3h5v5l-1.5-1.5-4.538 4.538-1.414-1.414L17.5 5.5 16 4V3zM4 5l6.5 6.5-1.414 1.415L3 6.415 4 5zm10.5 9.5L9 10 7.5 11.5 13 17l1.5-1.5zm-4.538-4.538L3 17h2v1l6.5-6.5-1.538-1.538zM16 16l1.5-1.5 3.5 3.5V19h-5v-1l1.5-1.5L16 16z" /></svg></button>
          <button className="ctrl-btn" onClick={() => onAdvance(-1)}><svg width="16" height="16" fill="currentColor" viewBox="0 0 24 24"><path d="M6 6h2v12H6zm3.5 6 8.5 6V6z" /></svg></button>
          <button className="play-main-btn" onClick={onTogglePlay}>{buffering ? <svg className="spin" width="12" height="12" fill="none" stroke="var(--bg)" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M21 12a9 9 0 11-6.219-8.56" /></svg> : playing ? <svg width="12" height="12" fill="var(--bg)" viewBox="0 0 24 24"><path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z" /></svg> : <svg width="12" height="12" fill="var(--bg)" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg>}</button>
          <button className="ctrl-btn" onClick={() => onAdvance(1)}><svg width="16" height="16" fill="currentColor" viewBox="0 0 24 24"><path d="M6 18l8.5-6L6 6v12zm10-12v12h2V6h-2z" /></svg></button>
          <button className={`ctrl-btn${repeat ? " active" : ""}`} onClick={onRepeat}><svg width="14" height="14" fill="currentColor" viewBox="0 0 24 24"><path d="M7 7h10v3l4-4-4-4v3H5v6h2V7zm10 10H7v-3l-4 4 4 4v-3h12v-6h-2v4z" /></svg></button>
        </div>
        <div className="prog-row">
          <span className="prog-time">{fmt(progress)}</span>
          <div className="prog-rail" onClick={handleSeek}><div className="prog-fill" style={{ width: `${pct}%` }} /></div>
          <span className="prog-time">{fmt(duration)}</span>
        </div>
      </div>
      <div className="pl-right">
        {playing && !buffering && <div className="eq" style={{ marginRight: 4 }}><div className="eq-b" /><div className="eq-b" /><div className="eq-b" /></div>}
        <button className="icon-btn" onClick={onFullscreen}><svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M8 3H5a2 2 0 0 0-2 2v3m18 0V5a2 2 0 0 0-2-2h-3m0 18h3a2 2 0 0 0 2-2v-3M3 16v3a2 2 0 0 0 2 2h3" /></svg></button>
        <div className="vol-wrap">
          <svg width="12" height="12" fill="currentColor" style={{ color: "var(--tx3)", flexShrink: 0 }} viewBox="0 0 24 24">{volume === 0 ? <path d="M16.5 12c0-1.77-1.02-3.29-2.5-4.03v2.21l2.45 2.45c.03-.2.05-.41.05-.63zm2.5 0c0 .94-.2 1.82-.54 2.64l1.51 1.51C20.63 14.91 21 13.5 21 12c0-4.28-2.99-7.86-7-8.77v2.06c2.89.86 5 3.54 5 6.71zM4.27 3L3 4.27 7.73 9H3v6h4l5 5v-6.73l4.25 4.25c-.67.52-1.42.93-2.25 1.18v2.06c1.38-.31 2.63-.95 3.69-1.81L19.73 21 21 19.73l-9-9L4.27 3zM12 4L9.91 6.09 12 8.18V4z" /> : volume < 50 ? <path d="M18.5 12c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM5 9v6h4l5 5V4L9 9H5z" /> : <path d="M3 9v6h4l5 5V4L7 9H3zm13.5 3c0-1.77-1.02-3.29-2.5-4.03v8.05c1.48-.73 2.5-2.25 2.5-4.02zM14 3.23v2.06c2.89.86 5 3.54 5 6.71s-2.11 5.85-5 6.71v2.06c4.01-.91 7-4.49 7-8.77s-2.99-7.86-7-8.77z" />}</svg>
...
export default function App() {
  ...
  const [results, setResults] = useState<{ tracks: Track[]; artists: Track[]; albums: Playlist[]; playlists: Playlist[] } | null>(null);
  const [current, setCurrent] = useState<Track | null>(null);
  const [playing, setPlaying] = useState(false);
  const [buffering, setBuffering] = useState(false);
  const [queue, setQueue] = useState<Track[]>([]);
  const [qIdx, setQIdx] = useState(0);
  const [liked, setLiked] = useState<Track[]>(() => lsGet("liked", []));
  const [recent, setRecent] = useState<Track[]>(() => lsGet("recent", []));
  const [charts, setCharts] = useState<Track[]>([]);
  const [featured, setFeatured] = useState<Playlist[]>([]);
  const [genres, setGenres] = useState<{ id: number; name: string; picture_medium?: string }[]>([]);
  const [selPl, setSelPl] = useState<Playlist | null>(null); const [plTracks, setPlTracks] = useState<Track[]>([]);
  const [selAlb, setSelAlb] = useState<Playlist | null>(null); const [albTracks, setAlbTracks] = useState<Track[]>([]);
  const [artist, setArtist] = useState<{ info: Track["artist"] & { picture_big?: string; nb_fan?: number }; top: Track[]; albums: Playlist[] } | null>(null);
  const [selGenre, setSelGenre] = useState<{ id: number; name: string } | null>(null); const [genTracks, setGenTracks] = useState<Track[]>([]);
  ...
  const [selCustomPl, setSelCustomPl] = useState<Playlist | null>(null);
  const [customPlTracks, setCustomPlTracks] = useState<Track[]>([]);
  const [showCreatePl, setShowCreatePl] = useState(false);
  const [newPlName, setNewPlName] = useState("");
  const [newPlPublic, setNewPlPublic] = useState(false);
  const [plSearch, setPlSearch] = useState("");
  const [addToPlMenu, setAddToPlMenu] = useState<{ track: Track; x: number; y: number } | null>(null);
  const [similar, setSimilar] = useState<Track[]>([]);
  const [similarLoading, setSimilarLoading] = useState(false);
  const [user, setUser] = useState<UserInfo | null>(() => { const uid = getUserId(); return uid ? { id: uid } : null; });
  const [authOpen, setAuthOpen] = useState(false);

  const ytRef = useRef<{
    getCurrentTime?: () => number; getDuration?: () => number; getPlayerState?: () => number;
    loadVideoById?: (id: string) => void; setVolume?: (v: number) => void;
    pauseVideo?: () => void; playVideo?: () => void; seekTo?: (t: number, b?: boolean) => void;
  } | null>(null);
  const qRef = useRef<Track[]>([]); const qIdxRef = useRef(0);
  const shuffleRef = useRef(shuffle); const repeatRef = useRef(repeat); const volRef = useRef(volume);
  const mediaSessionTrackRef = useRef<Track | null>(null);
  ...
  /* Sync to Supabase user_data when user is logged in (debounced) */
  const syncTimeoutRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const syncToCloud = useCallback((newLiked: Track[], newRecent: Track[], prefs: Record<string,unknown>) => {
    if (!getUserId()) return;
    if (syncTimeoutRef.current) clearTimeout(syncTimeoutRef.current);
    syncTimeoutRef.current = setTimeout(() => { userDataUpsert(newLiked, newRecent, prefs); }, 1500);
  }, []);

  const currentPrefs = useCallback(() => ({ darkMode, explicitFilter, audioQuality, crossfade, autoplay, volume }), [darkMode, explicitFilter, audioQuality, crossfade, autoplay, volume]);

  useEffect(() => { syncToCloud(liked, recent, currentPrefs()); }, [liked, recent, darkMode, explicitFilter, audioQuality, crossfade, autoplay, volume]);

  useEffect(() => { document.documentElement.setAttribute("data-theme", darkMode ? "dark" : "light"); }, [darkMode]);

  /* Load user data from cloud when signed in */
  const loadUserData = useCallback(async () => {
    const data = await userDataGet();
    if (!data) return;
    if (Array.isArray(data.liked) && data.liked.length > 0) setLiked(data.liked);
    if (Array.isArray(data.recent) && data.recent.length > 0) setRecent(data.recent);
    if (data.prefs && typeof data.prefs === "object") {
      const p = data.prefs as Record<string,unknown>;
      if (typeof p.darkMode === "boolean") setDarkMode(p.darkMode);
      if (typeof p.explicitFilter === "boolean") setExplicitFilter(p.explicitFilter);
      if (typeof p.audioQuality === "string") setAudioQuality(p.audioQuality);
      if (typeof p.crossfade === "boolean") setCrossfade(p.crossfade);
      if (typeof p.autoplay === "boolean") setAutoplay(p.autoplay);
      if (typeof p.volume === "number") { setVolume(p.volume); volRef.current = p.volume; ytRef.current?.setVolume?.(p.volume); }
    }
  }, []);

  /* Get profile info from Supabase user */
  const loadUserProfile = useCallback(async () => {
    ...
    try {
      ...
      if (data?.id) {
        ...
        setUser({ id: data.id, email: data.email, name: meta.full_name || meta.name || data.email?.split("@")[0], avatar_url: meta.avatar_url || meta.picture });
      }
    } catch {}
  }, []);
  ...
  useEffect(() => {
    const handle = (e: KeyboardEvent) => {
      ...
      if (e.code === "KeyL") { if (current) { if (!user) { setAuthOpen(true); } else { toggleLike(current); } } }
      if (e.code === "KeyS") setShuffle(s => !s);
      if (e.code === "KeyQ") setShowQueue(q => !q);
      if (e.code === "KeyB") setSidebarOpen(s => !s);
      if (e.code === "KeyF") setFullscreenOpen(f => !f);
  ...
  useEffect(() => {
    ...
    if (sleepMins) {
      sleepRef.current = setTimeout(() => { ytRef.current?.pauseVideo?.(); setSleepMins(null); toast("💤 Sleep timer — paused"); }, sleepMins * 60 * 1000);
    }
  ...
  useEffect(() => {
    ...
    const img = new Image(); img.crossOrigin = "anonymous"; img.src = current.album.cover_small;
    img.onload = () => {
      try {
  ...
  const initYT = () => {
    ...
    const YT = (window as Window & { YT?: { Player: new (id: string, opts: object) => {
      setVolume: (v: number) => void; loadVideoById: (id: string) => void; pauseVideo: () => void;
      playVideo: () => void; seekTo: (t: number, b?: boolean) => void; getCurrentTime: () => number;
      getDuration: () => number; getPlayerState: () => number;
    } }; PlayerState?: { PLAYING: number; PAUSED: number; BUFFERING: number; ENDED: number } }).YT;
    if (!YT) return;
    ytRef.current = new YT.Player("yt-mount", {
      height: "1", width: "1", videoId: "",
      playerVars: { autoplay: 1, controls: 0, rel: 0, playsinline: 1, enablejsapi: 1, origin: window.location.origin },
      events: {
        onReady: (e: { target: { setVolume: (v: number) => void } }) => e.target.setVolume(volRef.current),
        onStateChange: (e: { data: number; target: { seekTo: (t: number) => void; playVideo: () => void } }) => {
          const S = (window as Window & { YT?: { PlayerState?: { PLAYING: number; PAUSED: number; BUFFERING: number; ENDED: number } } }).YT?.PlayerState;
          if (!S) return;
          if (e.data === S.PLAYING) {
            setPlaying(true); setBuffering(false);
            const cur = mediaSessionTrackRef.current || qRef.current[qIdxRef.current];
            if (cur) applyMediaMeta(cur);
  ...
  const applyMediaMeta = (track: Track) => {
    if (!("mediaSession" in navigator) || !track) return;
    navigator.mediaSession.metadata = new MediaMetadata({
      title: track.title || "Unknown", artist: track.artist?.name || "Unknown",
      album: track.album?.title || "",
      artwork: [track.album?.cover_small, track.album?.cover_medium, track.album?.cover_big].filter(Boolean).map(src => ({ src: src!, type: "image/jpeg" })),
    });
  };

  const setMediaSession = (track: Track) => {
    if (!("mediaSession" in navigator)) return;
    mediaSessionTrackRef.current = track; applyMediaMeta(track);
  ...
  const doPlay = async (track: Track, list: Track[] = [], idx = 0) => {
  ...
  const handleSeek = useCallback((t: number) => { ytRef.current?.seekTo?.(t, true); }, []);
  const changeVol = useCallback((v: number) => { setVolume(v); volRef.current = v; ytRef.current?.setVolume?.(v); }, []);
  ...
  const handleDownload = useCallback(async (track: Track) => {
    ...
    try {
      ...
      a.href = url; a.download = `${track.artist?.name} - ${track.title} (30s preview).mp3`;
  ...
  const loadHome = async () => {
    ...
    try {
      ...
      setCharts(c.data || []);
      setGenres((g.data || []).filter((x: { id: number }) => x.id !== 0).slice(0, 12));
  ...
  const doSearch = async (e?: React.FormEvent) => {
    ...
    try {
      const [tr, ar, al, pl] = await Promise.all([dz(`/search/track?q=${encodeURIComponent(query)}&limit=15`), dz(`/search/artist?q=${encodeURIComponent(query)}&limit=6`), dz(`/search/album?q=${encodeURIComponent(query)}&limit=6`), dz(`/search/playlist?q=${encodeURIComponent(query)}&limit=6`)]);
      setResults({ tracks: tr.data || [], artists: ar.data || [], albums: al.data || [], playlists: pl.data || [] });
    } catch {}
    setLoading(false);
  };

  const openPlaylist = async (p: Playlist) => { setSelPl(p); setView("playlist"); setLoading(true); const d = await dz(`/playlist/${p.id}/tracks?limit=50`); setPlTracks(d.data || []); setLoading(false); };
  const openAlbum = async (a: Playlist) => { setSelAlb(a); setView("album"); setLoading(true); const d = await dz(`/album/${a.id}/tracks`); setAlbTracks(d.data || []); setLoading(false); };
  const openArtist = async (a: Track) => { setView("artist"); setLoading(true); const [i, t, alb] = await Promise.all([dz(`/artist/${a.id}`), dz(`/artist/${a.id}/top?limit=10`), dz(`/artist/${a.id}/albums?limit=10`)]); setArtist({ info: i, top: t.data || [], albums: alb.data || [] }); setLoading(false); };
  const openGenre = async (g: { id: number; name: string }) => { setSelGenre(g); setView("genre"); setLoading(true); const d = await dz(`/chart/${g.id}/tracks?limit=20`); setGenTracks(d.data || []); setLoading(false); };
  ...
  const signOut = async () => { await sbSignOut(); setUser(null); setPlaylists([]); toast("Signed out"); };
  ...
  const openCustomPlaylist = async (pl: Playlist) => {
    setSelCustomPl(pl); setView("custom_playlist"); setLoading(true);
    const tracks = await plGetTracks(pl.id); setCustomPlTracks(tracks); setLoading(false);
  };

  const getSimilar = async (track: Track) => {
    if (!track) return;
    setSimilarLoading(true); setSimilar([]); setView("similar");
    ...
    } catch { toast("⚠ Could not load similar tracks"); }
  ...
  const tp = {
    explicitFilter, currentId: current?.id, playing, buffering, liked, user,
    onPlay: doPlay, onLike: toggleLike, onSimilar: getSimilar, onDownload: handleDownload,
    onAuthRequired,
    onAddToPlaylist: (track: Track, e: React.MouseEvent) => {
  ...
  const userLabel = user?.name?.split(" ")[0] || user?.email?.split("@")[0] || "me";
  ...
  return (
    <> <style>{GLOBAL_CSS}</style>
      ...
      {/* TOASTS */}
      <div className="toast-area">{toasts.map(t => <div key={t.id} className="toast">{t.msg}</div>)}</div>

      {/* COMMAND PALETTE */}
      {cmdOpen && <CommandPalette onClose={() => setCmdOpen(false)} recent={recent} liked={liked} charts={charts} onPlay={doPlay} onNav={v => { setView(v); setCmdOpen(false); }} onAction={a => { if (a === "fullscreen") setFullscreenOpen(f => !f); if (a === "shuffle") setShuffle(s => !s); if (a === "darkmode") setDarkMode(d => !d); if (a === "sleep") handleSleep(); if (a === "lyrics") setView("lyrics"); }} />}

      {/* AUTH MODAL */}
      {authOpen && <AuthModal onClose={() => setAuthOpen(false)} />}

      {/* CREATE PLAYLIST MODAL */}
      {showCreatePl && <div className="pl-modal-backdrop" onClick={() => setShowCreatePl(false)}>
      ...
      {addToPlMenu && <><div style={{ position: "fixed", inset: 0, zIndex: 199 }} onClick={() => setAddToPlMenu(null)} /><div className="add-to-pl-menu" style={{ top: addToPlMenu.y, left: addToPlMenu.x }}>
        {playlists.length === 0 ? <div className="add-to-pl-item" style={{ color: "var(--tx3)", cursor: "default" }}>No playlists — create one first</div>
          : playlists.map(pl => <div key={pl.id} className="add-to-pl-item" onClick={() => { addToPlaylist(pl.id, addToPlMenu.track); setAddToPlMenu(null); }}><svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M3 6h18M3 12h18M3 18h18" /></svg>{pl.name}</div>)}
      </div></>}

      {/* FULLSCREEN */}
      {fullscreenOpen && <FullscreenView current={current} queue={queue} qIdx={qIdx} playing={playing} buffering={buffering} shuffle={shuffle} repeat={repeat} liked={liked} volume={volume} ytRef={ytRef} user={user} relatedTracks={relatedTracks} onClose={() => setFullscreenOpen(false)} onTogglePlay={togglePlay} onAdvance={advance} onSeek={handleSeek} onVolume={changeVol} onLike={toggleLike} onShuffle={() => { setShuffle(s => { toast(!s ? "Shuffle on" : "Shuffle off"); return !s; }); }} onRepeat={() => { setRepeat(r => { toast(!r ? "Repeat on" : "Repeat off"); return !r; }); }} onPlayFromQueue={(t, i) => { qIdxRef.current = i; setQIdx(i); doPlay(t, queue, i); }} onAuthRequired={onAuthRequired} />}

      {/* QUEUE PANEL */}
      {showQueue && <><div className="queue-overlay" onClick={() => setShowQueue(false)} /><div className="queue-panel">
        <div className="queue-hd"><span className="queue-title">Queue · <span style={{ fontFamily: "'Geist Mono',monospace", fontSize: 12, color: "var(--tx3)" }}>{queue.length} tracks</span></span><button className="icon-btn" onClick={() => setShowQueue(false)}><svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M18 6 6 18M6 6l12 12" /></svg></button></div>
        <div className="queue-body">{queue.length === 0 ? <div className="empty"><div className="empty-sub">queue is empty</div></div> : queue.map((t, i) => <div key={`${t.id}-${i}`} className={`queue-item${i === qIdx ? " current" : ""}`} onClick={() => { qIdxRef.current = i; setQIdx(i); doPlay(t, queue, i); }}><span className="queue-num">{i === qIdx && playing ? <div className="eq"><div className="eq-b" /><div className="eq-b" /><div className="eq-b" /></div> : i + 1}</span><img src={t.album?.cover_small || ""} alt="" /><div className="queue-item-meta"><div className={`queue-item-name${i === qIdx ? " active" : ""}`}>{t.title}</div><div className="queue-item-sub">{t.artist?.name}</div></div><span style={{ fontFamily: "'Geist Mono',monospace", fontSize: 10, color: "var(--tx3)", flexShrink: 0 }}>{fmt(t.duration)}</span></div>)}</div>
      </div></>}

      <div className={`shell${sidebarOpen ? "" : " nav-collapsed"}`}>
        {/* SIDEBAR */}
        <nav className="nav"><div className="nav-inner">
          <div className="nav-header"><div className="logo"><WaveLogo size={18} /><span className="logo-word">WAVE</span></div></div>
          <div className="nav-body">
            <div className="nav-section">
              ...
              ].map(n => <div key={n.v} className={`nav-item${view === n.v ? " active" : ""}`} onClick={() => setView(n.v)}>{n.ico} {n.label}{n.v === "liked" && liked.length > 0 && <span className="badge">{liked.length}</span>}</div>)}
            </div>
            <div className="nav-section">
              <div className="nav-section-label">Library</div>
              {liked.length === 0 ? <div style={{ padding: "6px 16px", fontSize: 11, color: "var(--tx3)", fontFamily: "'Geist Mono',monospace" }}>empty</div>
                : liked.slice(0, 10).map(t => <div key={t.id} className="nav-lib-track" onClick={() => doPlay(t)}><img src={t.album?.cover_small || ""} alt="" /><span>{t.title}</span>{current?.id === t.id && playing && <div className="eq" style={{ marginLeft: "auto", flexShrink: 0 }}><div className="eq-b" /><div className="eq-b" /><div className="eq-b" /></div>}</div>)}
            </div>
          </div>
          {current && <div className="nav-now-playing"><div className="nav-now-label">Now Playing</div><div className="nav-now-info"><img className="nav-now-img" src={current.album?.cover_small || ""} alt="" /><div style={{ overflow: "hidden" }}><div className="nav-now-title">{current.title}</div><div className="nav-now-artist">{current.artist?.name}</div></div></div></div>}
        </div></nav>
        ...
        <main className="main">
          <div className="topbar">
            <div className="topbar-left">
              <button className="icon-btn" onClick={() => setSidebarOpen(s => !s)}><svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2" /><path d="M9 3v18" /></svg></button>
              <button className="icon-btn" onClick={() => setView("home")}><svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M19 12H5M12 5l-7 7 7 7" /></svg></button>
            </div>
            ...
            <div className="topbar-right">
              ...
              <button className="icon-btn" onClick={() => setCmdOpen(c => !c)}><svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M18 3a3 3 0 0 0-3 3v12a3 3 0 0 0 3 3 3 3 0 0 0 3-3 3 3 0 0 0-3-3H6a3 3 0 0 0-3 3 3 3 0 0 0 3 3 3 3 0 0 0 3-3V6a3 3 0 0 0-3-3 3 3 0 0 0-3 3 3 3 0 0 0 3 3h12a3 3 0 0 0 3-3 3 3 0 0 0-3-3z" /></svg></button>
              <button className="icon-btn" onClick={() => setView("lyrics")}><svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M9 18V5l12-2v13" /><circle cx="6" cy="18" r="3" /><circle cx="18" cy="16" r="3" /></svg></button>
              <button className="icon-btn sleep-btn" onClick={handleSleep}><svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M12 22c5.523 0 10-4.477 10-10S17.523 2 12 2 2 6.477 2 12s4.477 10 10 10z"/><path d="M12 6v6l4 2"/></svg>{sleepMins && <span className="sleep-badge">{sleepMins}</span>}</button>
              <button className="icon-btn" onClick={() => setShowQueue(q => !q)}><svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M8 6h13M8 12h13M8 18h13M3 6h.01M3 12h.01M3 18h.01" /></svg></button>
              <button className="icon-btn" onClick={() => setDarkMode(d => !d)}>{darkMode ? <svg width="13" height="13" fill="currentColor" viewBox="0 0 24 24"><path d="M12 3a6 6 0 0 0 9 9 9 9 0 1 1-9-9z" /></svg> : <svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="5" /><path d="M12 1v2M12 21v2M4.22 4.22l1.42 1.42M18.36 18.36l1.42 1.42M1 12h2M21 12h2M4.22 19.78l1.42-1.42M18.36 5.64l1.42-1.42" /></svg>}</button>
              {user
                ? <button className="user-chip" onClick={signOut} title="Sign out">
                    <div className="user-avatar">{user.avatar_url ? <img src={user.avatar_url} alt="" /> : userInitial.toUpperCase()}</div>
                    {userLabel}
                  </button>
                : <button className="icon-btn" onClick={() => setAuthOpen(true)} title="Sign in"><svg width="13" height="13" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="8" r="4"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7"/></svg></button>}
            </div>
          </div>
          ...
          {!loading && view === "home" && <div>
            <div className="ph"><div className="pt">Overview</div><div className="ps">wave · deezer api · {new Date().toLocaleDateString("en-US", { weekday: "long", month: "short", day: "numeric" })}</div></div>
            {recent.length > 0 && <div className="sec"><div className="sec-hd"><span className="sec-lbl">Recently Played</span></div><div className="recent-strip">{recent.map(t => <div key={t.id} className="recent-item" onClick={() => doPlay(t)}><img src={t.album?.cover_medium || t.album?.cover_small || ""} alt="" /><div><div className="recent-item-name">{t.title}</div><div className="recent-item-sub">{t.artist?.name}</div></div></div>)}</div></div>}
            <div className="sec"><div className="sec-hd"><span className="sec-lbl">Top Charts</span><span className="sec-act" onClick={() => setView("browse")}>browse →</span></div><TTable tracks={charts.slice(0, 10)} {...tp} /></div>
            <div className="sec"><div className="sec-hd"><span className="sec-lbl">Genres</span></div><div className="ggrid">{genres.map(g => <div key={g.id} className="gcel" onClick={() => openGenre(g)}><div className="gcel-name">{g.name}</div>{g.picture_medium && <img className="gcel-img" src={g.picture_medium} alt="" />}</div>)}</div></div>
            {featured.length > 0 && <div className="sec"><div className="sec-hd"><span className="sec-lbl">Featured Albums</span></div><CardGrid items={featured} onClick={a => openAlbum(a as Playlist)} imgFn={a => (a as Playlist).cover_medium || ""} titleFn={a => (a as Playlist).title || ""} subFn={a => (a as Playlist & { artist?: { name: string } }).artist?.name || ""} /></div>}
          </div>}

          {!loading && view === "search" && results && <div>
            ...
            {results.artists?.length > 0 && <div className="sec"><div className="sec-hd"><span className="sec-lbl">Artists</span></div><CardGrid items={results.artists} onClick={a => openArtist(a as Track)} imgFn={a => (a as Track).artist?.picture_medium || ""} titleFn={a => (a as Track).artist?.name || String((a as Track).id)} subFn={a => fmtBig((a as Track & { nb_fan?: number }).nb_fan || 0) + " fans"} round /></div>}
            {results.albums?.length > 0 && <div className="sec"><div className="sec-hd"><span className="sec-lbl">Albums</span></div><CardGrid items={results.albums} onClick={a => openAlbum(a as Playlist)} imgFn={a => (a as Playlist).cover_medium || ""} titleFn={a => (a as Playlist).title || ""} subFn={a => (a as Playlist & { artist?: { name: string } }).artist?.name || ""} /></div>}
            {results.playlists?.length > 0 && <div className="sec"><div className="sec-hd"><span className="sec-lbl">Playlists</span></div><CardGrid items={results.playlists} onClick={a => openPlaylist(a as Playlist)} imgFn={a => (a as Playlist).picture_medium || ""} titleFn={a => (a as Playlist).title || ""} subFn={a => `${(a as Playlist).nb_tracks || 0} tracks`} /></div>}
          </div>}

          {!loading && view === "liked" && <div>
            <div className="ph">
              <div className="pt">Saved Tracks</div>
              <div className="ps">{liked.length} tracks saved{!user && " · sign in to save across devices"}</div>
            </div>
            {!user && <div style={{ padding: "12px 20px", background: "var(--bg2)", borderBottom: "var(--line)", display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
              <span style={{ fontSize: 12, color: "var(--tx3)", fontFamily: "'Geist Mono',monospace" }}>Sign in to sync your saved tracks and access them anywhere.</span>
              <button className="pl-create-btn" onClick={() => setAuthOpen(true)}>Sign in with Google</button>
            </div>}
            {liked.length === 0 ? <div className="empty">
              <div className="empty-title">Nothing saved yet</div>
              <div className="empty-sub">{user ? "Click ♥ on any track to save it" : "Sign in and click ♥ on any track"}</div>
            </div> : <>
              {/* Download section notice */}
              <div style={{ padding: "8px 18px", borderBottom: "var(--line)", display: "flex", alignItems: "center", gap: 8, background: "var(--bg2)" }}>
                <svg width="11" height="11" fill="none" stroke="currentColor" strokeWidth="2" style={{ color: "var(--tx3)", flexShrink: 0 }} viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" /><path d="M12 16v-4M12 8h.01" /></svg>
                <span className="dl-notice">Click the ⬇ icon on any track to download a free 30-second preview from Deezer. Login required.</span>
              </div>
              <TTable tracks={liked} {...tp} />
            </>}
          </div>}
          ...
          {view === "library" && <div>
            <div className="ph" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
              ...
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                ...
                {user ? <button className="pl-create-btn" onClick={() => setShowCreatePl(true)}><svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M12 5v14M5 12h14" /></svg>New</button>
                  : <button className="pl-create-btn" onClick={() => setAuthOpen(true)}>Sign in</button>}
              </div>
            </div>
            {(() => {
              ...
              return <div className="cgrid">{filtered.map(pl => <div key={pl.id} className="pl-card gc" onClick={() => openCustomPlaylist(pl)}>
                <div className="pl-card-icon gc-img" style={{ aspectRatio: 1 }}><svg width="28" height="28" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24"><path d="M3 6h18M3 12h18M3 18h18" /></svg></div>
                ...
                {user && pl.user_id === user.id && <button className="pl-card-delete" onClick={e => { e.stopPropagation(); deletePlaylist(pl.id); }}><svg width="11" height="11" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M18 6 6 18M6 6l12 12" /></svg></button>}
              </div>)}</div>;
            })()}
          </div>}

          {!loading && view === "custom_playlist" && selCustomPl && <div>
            <div className="ehero">
              <div className="ehero-img-cell" style={{ background: "var(--bg3)" }}><svg width="48" height="48" fill="none" stroke="var(--tx3)" strokeWidth="1.5" viewBox="0 0 24 24"><path d="M3 6h18M3 12h18M3 18h18" /></svg></div>
              <div className="ehero-info">
                <div className="entity-type">Playlist</div><div className="entity-name">{selCustomPl.name}</div>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                  <button className="play-hero-btn" onClick={() => customPlTracks.length && doPlay(customPlTracks[0], customPlTracks, 0)}><svg width="11" height="11" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg> Play All</button>
                  {user && <button className="similar-btn" style={{ marginTop: 0 }} onClick={() => { /* remove from playlist handled per-row */ }}>Manage</button>}
                </div>
              </div>
            </div>
          ...
          {!loading && view === "browse" && <div>
            <div className="ph"><div className="pt">Browse</div><div className="ps">genres & moods</div></div>
            <div className="sec"><div className="sec-hd"><span className="sec-lbl">All Genres</span></div><div className="ggrid">{genres.map(g => <div key={g.id} className="gcel" onClick={() => openGenre(g)}><div className="gcel-name">{g.name}</div>{g.picture_medium && <img className="gcel-img" src={g.picture_medium} alt="" />}</div>)}</div></div>
          </div>}

          {!loading && view === "genre" && selGenre && <div>
            <div className="ph" style={{ display: "flex", alignItems: "flex-start", flexDirection: "column", gap: 8 }}>
              ...
              <div className="pt">{selGenre.name}</div><div className="ps">top {genTracks.length} tracks</div>
            </div>
            <TTable tracks={genTracks} {...tp} />
          </div>}

          {!loading && view === "playlist" && selPl && <div>
            <div className="ehero">
              <div className="ehero-img-cell"><img className="ehero-img" src={selPl.picture_big || selPl.picture_medium || ""} alt="" /></div>
              <div className="ehero-info">
                <div className="entity-type">Playlist</div><div className="entity-name">{selPl.title}</div>
                <div className="entity-meta"><span className="emeta-item"><span className="lbl">tracks</span>{plTracks.length}</span>{selPl.fans && <span className="emeta-item"><span className="lbl">fans</span>{fmtBig(selPl.fans)}</span>}</div>
                <button className="play-hero-btn" onClick={() => plTracks.length && doPlay(plTracks[0], plTracks, 0)}><svg width="11" height="11" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg> Play All</button>
              </div>
            </div>
          ...
          {!loading && view === "album" && selAlb && <div>
            <div className="ehero">
              <div className="ehero-img-cell"><img className="ehero-img" src={(selAlb as Playlist & { cover_big?: string }).cover_big || selAlb.picture_medium || ""} alt="" /></div>
              <div className="ehero-info">
                <div className="entity-type">Album</div><div className="entity-name">{selAlb.title}</div>
                <div className="entity-meta">
                  <span className="emeta-item"><span className="lbl">artist</span>{(selAlb as Playlist & { artist?: { name: string } }).artist?.name}</span>
                  <span className="emeta-item"><span className="lbl">tracks</span>{albTracks.length}</span>
                  {(selAlb as Playlist & { release_date?: string }).release_date && <span className="emeta-item"><span className="lbl">year</span>{(selAlb as Playlist & { release_date?: string }).release_date!.slice(0, 4)}</span>}
                </div>
                <button className="play-hero-btn" onClick={() => albTracks.length && doPlay({ ...albTracks[0], album: selAlb as Track["album"] }, albTracks.map(t => ({ ...t, album: selAlb as Track["album"] })), 0)}><svg width="11" height="11" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg> Play All</button>
              </div>
            </div>
            <div style={{ padding: "8px 18px" }}><button className="back-btn" onClick={() => setView("search")}>← back</button></div>
            <TTable tracks={albTracks.map(t => ({ ...t, album: selAlb as Track["album"] }))} showAlbum={false} {...tp} />
          </div>}

          {!loading && view === "artist" && artist && <div>
            <div className="ehero">
              ...
              <div className="ehero-info">
                <div className="entity-type">Artist</div><div className="entity-name">{artist.info?.name}</div>
                <div className="entity-meta"><span className="emeta-item"><span className="lbl">fans</span>{fmtBig(artist.info?.nb_fan || 0)}</span><span className="emeta-item"><span className="lbl">albums</span>{artist.albums?.length}</span></div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 8 }}>
                  <button className="play-hero-btn" onClick={() => artist.top.length && doPlay(artist.top[0], artist.top, 0)}><svg width="11" height="11" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z" /></svg> Play Top Tracks</button>
                  <button className="similar-btn" onClick={() => artist.top[0] && getSimilar(artist.top[0])}><svg width="11" height="11" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="3" /><path d="M12 3a9 9 0 0 1 9 9M3 12a9 9 0 0 1 9-9" /></svg> Similar</button>
                </div>
              </div>
            </div>
            <div style={{ padding: "8px 18px" }}><button className="back-btn" onClick={() => setView("search")}>← back</button></div>
            <div className="sec"><div className="sec-hd"><span className="sec-lbl">Popular</span></div><TTable tracks={artist.top} {...tp} /></div>
            {artist.albums?.length > 0 && <div className="sec"><div className="sec-hd"><span className="sec-lbl">Discography <span style={{ color: "var(--tx3)" }}>{artist.albums.length}</span></span></div><CardGrid items={artist.albums} onClick={a => openAlbum(a as Playlist)} imgFn={a => (a as Playlist).picture_medium || ""} titleFn={a => (a as Playlist).title || ""} subFn={a => (a as Playlist & { release_date?: string }).release_date?.slice(0, 4) || ""} /></div>}
          </div>}

          {!loading && view === "settings" && <div>
            <div className="ph"><div className="pt">Settings</div><div className="ps">preferences & configuration{user ? ` · synced to account` : ""}</div></div>
            {/* Account section */}
            <div className="sec">
              <div className="sec-hd"><span className="sec-lbl">Account</span></div>
              <div className="setting-card" style={{ borderRight: "var(--line)", borderBottom: "var(--line)" }}>
                {user ? <div>
                  <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 14 }}>
                    <div className="user-avatar" style={{ width: 40, height: 40, fontSize: 16 }}>{user.avatar_url ? <img src={user.avatar_url} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} /> : userInitial.toUpperCase()}</div>
                    <div><div style={{ fontWeight: 600, fontSize: 14 }}>{user.name || userLabel}</div><div style={{ fontSize: 11, color: "var(--tx3)", fontFamily: "'Geist Mono',monospace" }}>{user.email}</div></div>
                  </div>
                  <div className="setting-card-desc">Preferences (dark mode, filters, volume) and library data are synced to your account.</div>
                  <button className="pl-create-btn" onClick={signOut}>Sign out</button>
                </div> : <div>
                  <div className="setting-card-title">Not signed in</div>
                  <div className="setting-card-desc">Sign in to sync your saved tracks, playlists, and preferences across devices. Also required to download previews.</div>
                  <button className="pl-create-btn" onClick={() => setAuthOpen(true)}>Sign in with Google</button>
                </div>}
              </div>
            </div>
            <div className="sec">
              ...
              <div className="settings-grid">
                <div className="setting-card"><div className="setting-card-title">Explicit Content Filter</div><div className="setting-card-desc">Hide tracks marked as explicit.</div><div className="toggle-wrap"><span style={{ fontSize: 12, fontFamily: "'Geist Mono',monospace", color: "var(--tx3)" }}>{explicitFilter ? "Filter ON" : "Filter OFF"}</span><Toggle checked={explicitFilter} onChange={v => { setExplicitFilter(v); toast(v ? "🔒 Explicit filter enabled" : "Explicit filter disabled"); }} /></div></div>
                <div className="setting-card"><div className="setting-card-title">Search Quality</div><div className="setting-card-desc">Controls how WAVE searches YouTube for audio.</div><select className="setting-select" value={audioQuality} onChange={e => { setAudioQuality(e.target.value); toast("Quality set to " + e.target.value); }}><option value="high">High — official audio priority</option><option value="standard">Standard — any audio match</option></select></div>
                <div className="setting-card"><div className="setting-card-title">Autoplay</div><div className="setting-card-desc">Automatically continue to the next track in queue.</div><div className="toggle-wrap"><span style={{ fontSize: 12, fontFamily: "'Geist Mono',monospace", color: "var(--tx3)" }}>{autoplay ? "Enabled" : "Disabled"}</span><Toggle checked={autoplay} onChange={v => { setAutoplay(v); toast(v ? "Autoplay enabled" : "Autoplay disabled"); }} /></div></div>
                <div className="setting-card"><div className="setting-card-title">Crossfade</div><div className="setting-card-desc">Smooth transition between tracks. (Visual indicator only.)</div><div className="toggle-wrap"><span style={{ fontSize: 12, fontFamily: "'Geist Mono',monospace", color: "var(--tx3)" }}>{crossfade ? "Enabled" : "Disabled"}</span><Toggle checked={crossfade} onChange={v => { setCrossfade(v); toast(v ? "Crossfade enabled" : "Crossfade disabled"); }} /></div></div>
              </div>
            </div>
            <div className="sec">
              <div className="sec-hd"><span className="sec-lbl">Appearance</span></div>
              <div className="settings-grid">
                <div className="setting-card"><div className="setting-card-title">Dark Mode</div><div className="setting-card-desc">Switch between light and dark themes.</div><div className="toggle-wrap"><span style={{ fontSize: 12, fontFamily: "'Geist Mono',monospace", color: "var(--tx3)" }}>{darkMode ? "Dark" : "Light"}</span><Toggle checked={darkMode} onChange={v => setDarkMode(v)} /></div></div>
                <div className="setting-card"><div className="setting-card-title">Sidebar</div><div className="setting-card-desc">Show or hide the navigation sidebar.</div><div className="toggle-wrap"><span style={{ fontSize: 12, fontFamily: "'Geist Mono',monospace", color: "var(--tx3)" }}>{sidebarOpen ? "Visible" : "Hidden"}</span><Toggle checked={sidebarOpen} onChange={v => setSidebarOpen(v)} /></div></div>
              </div>
            </div>
            <div className="sec">
              <div className="sec-hd"><span className="sec-lbl">Downloads</span></div>
              <div className="setting-card" style={{ borderRight: "var(--line)", borderBottom: "var(--line)" }}>
                <div className="setting-card-title">Preview Downloads</div>
                <div className="setting-card-desc">
                  WAVE lets you download a free 30-second audio preview of any track, provided by Deezer. These are not full songs — they are promotional preview clips.<br /><br />
                  To download a preview, click the <strong>⬇</strong> icon on any track row. You must be signed in to use this feature.
                </div>
                {!user && <button className="pl-create-btn" style={{ marginTop: 8 }} onClick={() => setAuthOpen(true)}>Sign in to enable downloads</button>}
              </div>
            </div>
            <div className="sec">
            ...
            <div className="sec">
              ...
              <div className="setting-card">
                <div className="about-logo"><WaveLogo size={22} /><div><div style={{ fontWeight: 700, fontSize: 15, letterSpacing: "-.02em" }}>WAVE</div><div className="about-version">v2.0.0 · Deezer API + YouTube IFrame API + Supabase</div></div></div>
                <div style={{ fontSize: 12, color: "var(--tx3)", fontFamily: "'Geist Mono',monospace", lineHeight: 1.6 }}>Music search and playback powered by the Deezer catalog. Audio delivered via YouTube. Account data stored securely in Supabase.</div>
        ...
        {/* PLAYER BAR */}
        <PlayerBar current={current} playing={playing} buffering={buffering} shuffle={shuffle} repeat={repeat} liked={liked} volume={volume} ytRef={ytRef} user={user} onTogglePlay={togglePlay} onAdvance={advance} onSeek={handleSeek} onVolume={changeVol} onLike={toggleLike} onShuffle={() => { setShuffle(s => { toast(!s ? "Shuffle on" : "Shuffle off"); return !s; }); }} onRepeat={() => { setRepeat(r => { toast(!r ? "Repeat on" : "Repeat off"); return !r; }); }} onFullscreen={() => setFullscreenOpen(true)} onAuthRequired={onAuthRequired} />

        {/* MOBILE BOTTOM NAV */}
        <nav className="mobile-nav">
          {[{ v: "home", label: "Home", ico: <svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg> }, { v: "browse", label: "Browse", ico: <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg> }, { v: "liked", label: "Saved", ico: <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg> }, { v: "lyrics", label: "Lyrics", ico: <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M9 18V5l12-2v13"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="16" r="3"/></svg> }, { v: "settings", label: "More", ico: <svg width="18" height="18" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><circle cx="12" cy="12" r="1"/><circle cx="19" cy="12" r="1"/><circle cx="5" cy="12" r="1"/></svg> }].map(n => <button key={n.v} className={`mobile-nav-item${view === n.v ? " active" : ""}`} onClick={() => setView(n.v)}>{n.ico}{n.label}</button>)}
        </nav>
      </div>
    </>
  );
